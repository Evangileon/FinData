...
namespace FinData
{
    ...
    public class FxjData
    {
        ...
        public int  InsertStockDataToSQLDb(string dataType,string CodeStr,string[,] CodeList,int iRecordCount)
        {
        ...
        public void InsertStockDataToSQLDb_hq(string Market,int iRecordCount)
        {
        ...
        struct fileStruct
        {
            ...
            public fileStruct(DataTypes fileType)
            {
                ...
                switch (fileType)
                {
                    ...
                    #region 代码表STKINFO60.DAT//代码的拼音是乱码，有可能拼音简写未存盘，而是由大智慧软件另行计算。
                    case DataTypes.dm:
        ...
        private string[,] GetData(DataTypes dataType,string code,string newFileName,int iRecordCount) //读取数据，重载
        {
            ...
            if (fxjFileStruct.isIndexDataStruct == true)
            {
                ...
                try
                {
                    ...
                    int iiRecordCount = 0;//读取的记录个数
                    if (iRecordCount == 0) iRecordCount = recordList.Count;
                    //if (recordList.Count <= iRecordCount) iRecordCount = recordList.Count;
                    ...
                    for (int i = recordList.Count-1; i >= 0; i--)
                    {
                        string[] record0 = (string[])recordList[i];
                            for (int j = 0; j < fieldCounts; j++)
                            {
                                records[iiRecordCount, j] = record0[j];
                            }
                        iiRecordCount = iiRecordCount + 1;
                        if (iiRecordCount == iRecordCount) break;    
                    }
                            ...
                            for (int i = 0; i < secCounts && hasCode==false; i++)
                            {
                                ...
                                if (fxjFileStruct.codeIsLong == false && code == market + code0 || fxjFileStruct.codeIsLong == true && code == code0)
                                {
                                    ...
                                    for (int iField = 0; iField < fieldCounts; iField++)
                                    {
                                        ...
                                        //{
                                        //    doubleTemp = (double)br.ReadSingle();
                                        //    Console.WriteLine((fs.Position-4).ToString() + ":" + doubleTemp.ToString());
                                        //}
                                        ...
                                        switch (fxjFileStruct.fields[iField, 2].ToLower())
                                        {
                                            ...
                                            case "single":
                                                //int temp = 0;
                                                //if (fxjFileStruct.fields[iField, 0].ToLower() == "mcsl5")
                                                //    temp = 1;                                               
