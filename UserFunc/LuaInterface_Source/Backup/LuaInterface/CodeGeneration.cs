namespace LuaInterface
{
    ...
    internal class CodeGeneration
    {
        ...
        public void GenerateClass(Type klass, out Type newType, out Type[][] returnTypes)
        {
            ...
            {
                builder = this.newModule.DefineType(str, TypeAttributes.Public, typeof(object), new Type[] { klass, typeof(ILuaGeneratedType) });
            }
            else
            {
                builder = this.newModule.DefineType(str, TypeAttributes.Public, klass, new Type[] { typeof(ILuaGeneratedType) });
            }
            ...
            ILGenerator iLGenerator = builder.DefineConstructor(MethodAttributes.Public, CallingConventions.Standard, new Type[] { typeof(LuaTable), typeof(Type[][]) }).GetILGenerator();
            ...
            foreach (MethodInfo info in methods)
            {
                if (klass.IsInterface)
                {
                    this.GenerateMethod(builder, info, MethodAttributes.VtableLayoutMask | MethodAttributes.HideBySig | MethodAttributes.Virtual, methodIndex, field, builder3, false, out returnTypes[methodIndex]);
                    methodIndex++;
                }
                else if ((!info.IsPrivate && !info.IsFinal) && info.IsVirtual)
                {
                    this.GenerateMethod(builder, info, (info.Attributes | MethodAttributes.VtableLayoutMask) ^ MethodAttributes.VtableLayoutMask, methodIndex, field, builder3, true, out returnTypes[methodIndex]);
                    methodIndex++;
                }
            }
        ...
        private void GenerateMethod(TypeBuilder myType, MethodInfo method, MethodAttributes attributes, int methodIndex, FieldInfo luaTableField, FieldInfo returnTypesField, bool generateBase, out Type[] returnTypes)
        {
            ...
            if (generateBase)
            {
                ILGenerator generator = myType.DefineMethod("__luaInterface_base_" + method.Name, MethodAttributes.VtableLayoutMask | MethodAttributes.HideBySig | MethodAttributes.Private, returnType, parameterTypes).GetILGenerator();
